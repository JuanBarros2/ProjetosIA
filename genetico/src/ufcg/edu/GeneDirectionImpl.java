...
public class GeneDirectionImpl implements Gene {
    ...
    private Integer RATE_OF_MEDIUM_MUTATION = Utils.RATE_OF_MEDIUM_MUTATION;
    ...
    public boolean doMutation() {
        bollean mutated = randomMutation() | mediumMutation() | smallMutation();
        return mutated;
    }
    ...
    private boolean mediumMutation() {
        if (Utils.willMutate(RATE_OF_MEDIUM_MUTATION)) {
            Integer change = Utils.generateRandominInterval(MED_MUTATION_VARIATION);
    ...
    private boolean smallMutation() {
        if (Utils.willMutate(RATE_OF_SMALL_MUTATION)) {
            Integer change = Utils.generateRandominInterval(SMALL_MUTATION_VARIATION);
            setForce(this.force + change);
            return true;
        }
        return false;
