...
particao <- createDataPartition(dadosBrutos$Price, p = .999, 
                                     list = FALSE, 
                                     times = 1)
...
```{r treinandoModelos}
config <- trainControl(method = "repeatedcv", number = 5, repeats = 2)
...
```{r}
postResample(pred = knnPredic, obs = dadosTeste$Price)
```

```{r}
postResample(pred = regLinearPredic, obs = dadosTeste$Price)
```

```{r}
postResample(pred = id3Predic, obs = dadosTeste$Price)
```

```{r}
postResample(pred = xgbPredic, obs = dadosTeste$Price)
```
